...
- 首轮优先建立“五下、六上 × 语文/数学/英语 × 每科约 10 套 papers”的题库体量，并优先找出对应 D5 基准卷候选；达到该目标后继续后台处理全库，不能自动停机。
- 必须实现 `paper-bank status`、本地 `dashboard.html`、日志滚动、Windows keep-awake、防睡眠验证和 Windows 任务计划自启/重试。
